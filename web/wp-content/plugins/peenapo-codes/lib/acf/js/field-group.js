	...
	*/
	...
	acf.conditional_logic = {
		...
		render : function( $field ){
			...
			$.each( $ancestors, function( i ){
				...
				$(this).children('.field').each(function(){
					...
					if( this_type == 'select' || this_type == 'checkbox' || this_type == 'true_false' || this_type == 'radio' || this_type == 'radio_image' )
					{
						choices.push({
							value	: this_id,
							label	: this_label,
		...
		change_trigger : function( $select ){
			...
			else if( type == "select" || type == "checkbox" || type == "radio" || type == "radio_image" )
			{
