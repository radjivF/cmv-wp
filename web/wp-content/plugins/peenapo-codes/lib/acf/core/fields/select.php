...
class acf_field_select extends acf_field
{
	...
	*/
	...
	function __construct()
	{
		...
		add_filter('acf/update_field/type=radio_image', array($this, 'update_field_radio_image'), 5, 2);
	}
...
<?php
	...
	*/
    ...
    function update_field_radio_image( $field ) {
		...
		foreach( explode( "\n", $field['choices'] ) as $row ) {
			
			$parse_row = explode( ':', $row );
			if( isset( $parse_row[1] ) ) {
				$parse_rest = explode( '|', $parse_row[1] );
				
				$new_choices[trim($parse_row[0])] = array(
					'label' => trim($parse_rest[0]),
					'img' => trim($parse_rest[1])
				);
			}
		}
		
		if( ! count( $new_choices ) ) { return $field; }
