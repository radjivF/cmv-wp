...
class acf_field_radio_image extends acf_field
{
	/*
	*  __construct
	*
	...
	*/
	...
	function __construct()
	{
		...
		$this->label = __("Radio Button Image",'acf');
	...
	function create_field( $field ) {
		...
		if( is_array($field['choices']) )
		{
			foreach( $field['choices'] as $key => $value )
			{
				...
				// HTML
				$e .= '
				<li class="acf-radio-image' . (empty($value['img']) ? ' default' : '') . ($active ? ' active' : '') . '" title="' . $value['label'] . '">
					<label>
	...
	*/
	...
	function create_options( $field )
	{
		...
		{		
			...
			{
				$field['choices'][ $k ] = $k . ' : ' . $v['label'] . ( !empty( $v['img'] ) ? ' | ' : '' ) . $v['img'];
			}
		...
		<tr class="field_option field_option_<?php echo $this->name; ?>">
			<td class="label">
				...
				<br />
				<?php _e("value : Label | layout_image/container.png",'acf'); ?><br />
				<?php _e("value2 : Label 2 | layout_image/default.png",'acf'); ?><br />
				</p>
			</td>
			<td>
				<?php
