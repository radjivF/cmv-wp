...
if ( ! defined( 'ABSPATH' ) || class_exists( 'WPGitHubUpdater' ) || class_exists( 'WP_Peenapo_Codes_GitHub_Updater' ) )
	return;
 ...
 */
class WP_Peenapo_Codes_GitHub_Updater {
    ...
    /**
	 * @var $githubAPIResult store the data fetched from GitHub
	 * @access private
	 */
	...
	public function get_new_version() {
		...
		if ( $this->overrule_transients() || ( !isset( $version ) || !$version || '' == $version ) ) {
			...
			$raw_response = $this->remote_get( trailingslashit( $this->config['api_url'] ) . 'contents/' . basename( $this->config['slug'] ) );
            ...
            if( isset( $raw_response['body'] ) ) {
                ...
                $decode_response = json_decode( $raw_response['body'] );
                if( isset( $decode_response->content ) ) {
                    $readme_content = base64_decode( $decode_response->content );
                }
            }

            if (!empty($readme_content)) {
                preg_match( '#^\s*Version\:\s*(.*)$#im', $readme_content, $matches );
            }
	...
	public function remote_get( $query ) {
        ...
        //$this->config['readme']
        //https://api.github.com/repos/Peenapo/peenapo-page-builder/contents/README.md?access_token=322baec1ce5ed1812a2c6b1283ac9ae6b97b39cb        
    ...
    private function getRepoReleaseInfo() {
        ...
        $url = 'https://api.github.com/repos/' . $this->config['username'] . '/' . $this->config['repo'] . '/releases';
	...
	public function api_check( $transient ) {
		...
		if ( 1 === $update ) {
			...
			$response->plugin = sanitize_title( $this->config['name'] );
            ...
            if ( !empty( $this->config['access_token'] ) ) {
                ...
                $response->package = add_query_arg( array( "access_token" => $this->config['access_token'] ), $package );
            }
	...
	public function upgrader_post_install( $true, $hook_extra, $result ) {
		...
		$fail  = __( 'The plugin has been updated, but could not be reactivated. Please reactivate it manually.', 'peenapo_page_puilder' );
		$success = __( 'Plugin reactivated successfully.', 'peenapo_page_puilder' );
