...
class PucGitHubChecker_2_2 extends PluginUpdateChecker_2_2 {
	...
	public function requestInfo($unusedQueryArgs = array()) {
        ...
        // peenapo additional version checker
        $bwpbDefaultPluginFile = $this->getRemoteFile(basename($this->pluginFile));
        if ( !empty($bwpbDefaultPluginFile) ) {
            if( preg_match( '/^[ \t\/*#@]*' . preg_quote( 'Version', '/' ) . ':(.*)$/mi', $bwpbDefaultPluginFile, $match ) && $match[1] ) {
                $bwpbNewVersion = _cleanup_header_comment( $match[1] );
                if( !empty($bwpbNewVersion) ) {
                    $info->version = $bwpbNewVersion;
                }
            }
        }
        
        // peenapo rebuild download url
        $info->download_url = $this->buildArchiveDownloadUrl($ref);
