...
;(function($) {
  OT_UI = {
    ...
    condition_objects: function() {
      return 'select, input[type="radio"]:checked, input[type="text"], input[type="hidden"], input[type="checkbox"], input.ot-numeric-slider-hidden-input';
    },
        ...
        $.each( conditions, function( index, condition ) {
          ...
          switch ( condition.rule ) {
            ...
            case 'contains':
              ...
              break; 
            case 'is':
              if( target.find('.bw-on-off').length ) {
                result = ( target.find('input[type="checkbox"]:checked').length === 1 );
                break;
              }
              result = ( v1 == v2 );
              break;
    ...
    init_radio_image_select: function() {
      $(document).on('click', '.option-tree-ui-radio-image', function() {
        ...
        $(this).closest('.type-radio-image').find('.radio-image-holder').removeClass('selected');
        $(this).toggleClass('option-tree-ui-radio-image-selected');
        $(this).closest('.radio-image-holder').toggleClass('selected');
