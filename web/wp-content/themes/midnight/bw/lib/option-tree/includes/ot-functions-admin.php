...
if ( ! function_exists( 'ot_export_php_settings_array' ) ) {
  ...
  function ot_export_php_settings_array( $to_file = false ) {
    ...
    if ( isset( $option_tree_settings['sections'] ) ) {
      foreach( $option_tree_settings['sections'] as $value ) {
        $_id = isset( $value['id'] ) ? $value['id'] : '';
        $_title = ot_I18n_string( isset( $value['title'] ) ? str_replace( "'", "\'", $value['title'] ) : '' );
        $_icon = isset( $value['icon'] ) ? $value['icon'] : '';
        $sections.= "
      array(
        'id'          => '$_id',
        'title'       => $_title,
  ...
  \$ot_has_custom_theme_options = true;
    ...
    if( ! $to_file ) {
        ...
        if ( is_writable( $to_file ) ) {
            ...
            $wp_filesystem->put_contents( $to_file, $content, FS_CHMOD_FILE );
            //file_put_contents( $to_file, $content );
            
            add_action( 'admin_notices', array( 'Bw_meta', 'ot_export_success' ) );
        }  else {
            add_action( 'admin_notices', array( 'Bw_meta', 'ot_export_fail' ) );
        }
    }
  }
...
if ( ! function_exists( 'ot_option_types_array' ) ) {
  ...
  function ot_option_types_array() {
    ...
    return apply_filters( 'ot_option_types_array', array( 
      ...
      /* bw includes */
      'bw-import-data'            => __('Bw import button', 'option-tree'),
      'bw-select-font'            => __('Bw select font', 'option-tree'),
      'bw-text-content'           => __('Bw text content', 'option-tree'),
      'bw-on-off'                 => __('Bw on/off button', 'option-tree')
    ) );
  ...
  function ot_sections_view( $name, $key, $section = array() ) {
    ...
    <div class="option-tree-setting is-section">
      ...
      <div class="option-tree-setting-body">
        ...
        <div class="format-settings">
          <div class="format-setting type-text">
            <div class="description">' . __( '<strong>Icon Class</strong>: Font Awesome icon class.', 'option-tree' ) . '</div>
            <div class="format-setting-inner">
              <input type="text" name="' . esc_attr( $name ) . '[' . esc_attr( $key ) . '][icon]" value="' . ( isset( $section['icon'] ) ? esc_attr( $section['icon'] ) : '' ) . '" class="widefat option-tree-ui-input section-icon" autocomplete="off" />
            </div>
          </div>
        </div>
      </div>
