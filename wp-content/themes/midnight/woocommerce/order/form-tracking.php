...
<form action="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" method="post" class="track_order<?php echo ! is_user_logged_in() ? ' bw-track-order' : ''; ?>">

    <?php if( ! is_user_logged_in() ) { get_template_part('templates/page-title'); } ?>

	<p class="bw-sub-title"><?php _e( 'To track your order please enter your Order ID in the box below and press the "Track" button. This was given to you on your receipt and in the confirmation email you should have received.', 'woocommerce' ); ?></p>
